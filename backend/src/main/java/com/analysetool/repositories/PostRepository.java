...
public interface PostRepository extends JpaRepository<Post, Long> {
   ...
   @Query("SELECT p FROM Post p where :id IN (SELECT user.id FROM AuthorsRelationships a JOIN WPTerm term ON a.authorTerm = term.id JOIN WPUser user ON user.nicename = term.slug WHERE user.id=:id) AND p.status='publish' AND (p.type='post' OR p.type='video')")
   List<Post> findByAuthor(long id);

   @Query("SELECT p.id FROM Post p where :id IN (SELECT user.id FROM AuthorsRelationships a JOIN WPTerm term ON a.authorTerm = term.id JOIN WPUser user ON user.nicename = term.slug WHERE user.id=:id) AND p.status='publish' AND (p.type='post' OR p.type='video')")
   List<Long> findPostIdsByUserId(Long id);

   @Query("SELECT p.id FROM Post p WHERE :userId IN (SELECT user.id FROM AuthorsRelationships a JOIN WPTerm term ON a.authorTerm = term.id JOIN WPUser user ON user.nicename = term.slug WHERE user.id=:userId) AND p.status = 'publish' AND (p.type='post' OR p.type='video')")
   ...
   @Query("SELECT p FROM Post p WHERE :authorId IN (SELECT user.id FROM AuthorsRelationships a JOIN WPTerm term ON a.authorTerm = term.id JOIN WPUser user ON user.nicename = term.slug WHERE user.id=:authorId) AND DATE(p.date) = DATE(:date) AND p.status='publish' AND p.type='post'")
   ...
   @Query("SELECT p FROM Post p WHERE p.type='event' AND p.status='publish' AND p.title LIKE %:search% AND :authorId IN (SELECT user.id FROM AuthorsRelationships a JOIN WPTerm term ON a.authorTerm = term.id JOIN WPUser user ON user.nicename = term.slug WHERE user.id=:authorId)")
