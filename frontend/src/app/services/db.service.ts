...
export class DbService {
  ...
  async getAllTagsWithCountAndRelevance(){
    return await fetch(DbService.getUrl(dbUrl.GET_ALL_TAGS_WITH_COUNT_AND_RELEVANCE), {credentials: "include"}).then(res => res.json());
  }
  ...
  async getUserClicks(id : string){
    return fetch(DbService.getUrl(dbUrl.GET_USER_CLICKS) + id, {credentials: "include"}).then(res => res.json());
  }
  ...
  async getOriginMapAll(){
    return await fetch(DbService.getUrl(dbUrl.GET_USER_ALL_ORIGIN_MAP), {credentials: "include"}).then(res => res.json());
  }
  ...
  async getClicksByTimeAll() : Promise<number[]>{
    return await fetch(DbService.getUrl(dbUrl.GET_USER_ALL_VIEWS_PER_HOUR), {credentials: "include"}).then(res => res.json());
  }
}
