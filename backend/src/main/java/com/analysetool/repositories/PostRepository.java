...
public interface PostRepository extends JpaRepository<Post, Long> {
   ...
   @Query("SELECT p, SUM(ps.clicks) AS totalClicks FROM Post p JOIN PostStats ps ON ps.artId=p.id WHERE p.title LIKE %:title% AND p.status=:status AND p.type=:type ORDER BY totalClicks DESC")
