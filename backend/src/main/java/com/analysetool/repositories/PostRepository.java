...
public interface PostRepository extends JpaRepository<Post, Long> {
   ...
   @Query("SELECT p FROM Post p where :id IN (SELECT user.id FROM AuthorsRelationships a JOIN WPTerm term ON a.authorTerm = term.id JOIN WPUser user ON user.nicename = term.slug) AND p.status='publish' AND (p.type='post' OR p.type='video')")
   List<Post> findByAuthor(long id);

   @Query("SELECT p.id FROM Post p where :id IN (SELECT user.id FROM AuthorsRelationships a JOIN WPTerm term ON a.authorTerm = term.id JOIN WPUser user ON user.nicename = term.slug) AND p.status='publish' AND (p.type='post' OR p.type='video')")
   List<Long> findPostIdsByUserId(Long id);

   @Query("SELECT p.id FROM Post p WHERE :userId IN (SELECT user.id FROM AuthorsRelationships a JOIN WPTerm term ON a.authorTerm = term.id JOIN WPUser user ON user.nicename = term.slug) AND p.status = 'publish' AND (p.type='post' OR p.type='video')")
   ...
   @Query("SELECT p FROM Post p LEFT JOIN wp_term_relationships wtr ON wtr.objectId = p.id LEFT JOIN WpTermTaxonomy wtt ON wtr.termTaxonomyId = wtt.termTaxonomyId LEFT JOIN WPTerm wpt ON wpt.id = wtt.termId WHERE p.status= 'publish' AND (p.type='post' OR p.type='video') AND :userId IN (SELECT user.id FROM AuthorsRelationships a JOIN WPTerm term ON a.authorTerm = term.id JOIN WPUser user ON user.nicename = term.slug) AND (p.title LIKE %:search% OR p.content LIKE %:search%) AND wpt.slug =:filter ORDER BY p.date DESC")
   List<Post> findByAuthorPageable(long userId, String search, String filter, Pageable pageable);

   @Query("SELECT p FROM Post p WHERE p.status= 'publish' AND (p.type='post' OR p.type='video') AND :userId IN (SELECT user.id FROM AuthorsRelationships a JOIN WPTerm term ON a.authorTerm = term.id JOIN WPUser user ON user.nicename = term.slug) AND (p.title LIKE %:search% OR p.content LIKE %:search%) ORDER BY p.date DESC")
   ...
   @Query("SELECT p FROM Post p WHERE :authorId IN (SELECT user.id FROM AuthorsRelationships a JOIN WPTerm term ON a.authorTerm = term.id JOIN WPUser user ON user.nicename = term.slug) AND DATE(p.date) = DATE(:date) AND p.status='publish' AND p.type='post'")
   ...
   @Query("SELECT p FROM Post p WHERE p.type='event' AND p.status='publish' AND p.title LIKE %:search% AND :authorId IN (SELECT user.id FROM AuthorsRelationships a JOIN WPTerm term ON a.authorTerm = term.id JOIN WPUser user ON user.nicename = term.slug)")
