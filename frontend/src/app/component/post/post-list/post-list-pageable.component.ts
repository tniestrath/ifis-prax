...
export class PostListPageableComponent extends DashListPageableComponent<Post, PostListItemComponent>{
  ...
  override ngOnInit(): void {
    ...
    this.input_search_cb = (event: { target: { value: string; }; }) => {
      ...
      this.reload(this.db.getPostsAllPaged(this.pageIndex, this.pageSize, "date", this.active_filter, this.search_text), PostListItemComponent);
    };
    this.input_all_cb = () => {
      this.pageIndex = 0;
      this.active_filter = " ";
...
export class PodcastListComponent extends PostListPageableComponent{
  ...
  override ngOnInit() {
    ...
    this.load(this.db.getPodcastsAll(), PostListItemComponent);
...
export class UserPostListComponent extends PostListPageableComponent{
  ...
  override ngOnInit() {
    ...
    this.input_search_cb = (event: { target: { value: string; }; }) => {
      this.search_text = event.target.value;
      this.load(this.db.getUserPostsPaged(SysVars.USER_ID, 0, 20, this.active_filter, this.search_text), PostListItemComponent);
    }
    this.input_all_cb = () => {
      this.active_filter = " ";
      this.load(this.db.getUserPostsPaged(SysVars.USER_ID, 0, 20, this.active_filter, this.search_text), PostListItemComponent);
...
export class EventListComponent extends PostListPageableComponent{
  ...
  override ngOnInit() {
    ...
    this.input_search_cb = (event: { target: { value: string; }; }) => {
      this.search_text = event.target.value;
      this.reload(this.db.getEventsLikePostsPaged( 0, 20, this.active_filter, this.search_text), PostListItemComponent);
    }
    this.input_all_cb = () => {
      this.active_filter = " ";
      this.reload(this.db.getEventsLikePostsPaged( 0, 20, this.active_filter, this.search_text), PostListItemComponent);
