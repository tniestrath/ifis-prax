...
public interface FinalSearchStatRepository extends JpaRepository<FinalSearchStat, Long> {
    ...
    @Query("SELECT s.searchQuery, COUNT(s.searchQuery) AS count FROM FinalSearchStat s WHERE s.searchQuery NOT IN (SELECT DISTINCT b.search FROM BlockedSearches b) GROUP BY s.searchQuery ORDER BY count, s.searchQuery DESC")
    List<Tuple> getQueriesAndCountsDESC(Pageable pageable);

    @Query("SELECT s.searchQuery, COUNT(s.searchQuery) AS count FROM FinalSearchStat s WHERE s.searchQuery NOT IN (SELECT DISTINCT b.search FROM BlockedSearches b) GROUP BY s.searchQuery ORDER BY count, s.searchQuery ASC")
    List<Tuple> getQueriesAndCountsASC(Pageable pageable);

    @Query("SELECT s.searchQuery, MAX(s.foundAnbieterCount + s.foundArtikelCount + s.foundBlogCount + s.foundEventsCount + s.foundNewsCount + s.foundPodcastCount + s.foundRatgeberCount+ s.foundWhitepaperCount) AS count FROM FinalSearchStat s WHERE s.searchQuery NOT IN (SELECT DISTINCT b.search FROM BlockedSearches b) GROUP BY s.searchQuery ORDER BY count, s.searchQuery ASC")
    ...
    @Query("SELECT s.searchQuery, COUNT(s.searchQuery) AS count FROM FinalSearchStat s INNER JOIN FinalSearchStatDLC dlc ON s.id=dlc.finalSearchId WHERE s.searchQuery NOT IN (SELECT DISTINCT b.search FROM BlockedSearches b) GROUP BY s.searchQuery ORDER BY count,s.searchQuery DESC")
    List<Tuple> getQueriesAndCountsSSDESC(Pageable pageable);

    @Query("SELECT s.searchQuery, COUNT(s.searchQuery) AS count FROM FinalSearchStat s INNER JOIN FinalSearchStatDLC dlc ON s.id=dlc.finalSearchId WHERE s.searchQuery NOT IN (SELECT DISTINCT b.search FROM BlockedSearches b) GROUP BY s.searchQuery ORDER BY count, s.searchQuery ASC")
